...
async def SendMessage(message: Message) -> Message | MessageInfo | None:
    ...
    try:
        ...
        print('Request timed out - server may be processing in background')
        # Return a placeholder since processing may continue in background
        return MessageInfo(
...
async def GetProcessingMessages():
    ...
    try:
        ...
        if response.result:
            ...
            return {str(k): str(v) for k, v in response.result}
        return {}
    except Exception as e:
    ...
    try:
        if conversation_id:
            ...
            if not messages:
                ...
                state.messages = [
                    convert_message_to_state(x) for x in messages]
