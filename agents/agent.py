...
from typing import Any, AsyncIterable, Dict, List, Optional, TypedDict, Union
...
class ElementAgent:
    ...
    SYSTEM_INSTRUCTION = (
        ...
        "\n\n当用户提到任何元素时，不论是常见还是罕见的元素，你必须准确识别它们。例如在'给我氢和氧的信息'这个查询中，你必须同时识别'氢'和'氧'这两个元素，并且只识别这两个元素。"
        ...
        "\n5. 对于含糊不清的查询，宁可返回'无法识别元素'，也不要返回用户未明确提及的元素"
    )
    ...
    def invoke(self, query: str, sessionId: str) -> ElementAPIResponse:
        ...
        try:
            ...
            self.graph.invoke({"input": enhanced_query}, config=cast(
                RunnableConfig, config_dict))

            # 使用工具处理响应
            final_response = self.get_agent_response(config_dict)
    ...
    async def stream(self, query: str, sessionId: str) -> AsyncIterable[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"[SessionID: {sessionId}] 查询改写为: \"{enhanced_query}\"")
            # 返回初始响应
            response = {"content": "正在分析您的查询...",
                        "is_task_complete": False}
            yield response

            # 使用LangGraph模型进行流式处理
            element_count = 0
            for chunk in self.graph.stream({"input": enhanced_query}, config=cast(RunnableConfig, config_dict)):
                if "messages" in chunk:
                    ...
                    if hasattr(message, "content"):
                        if isinstance(message, AIMessage):
                            ...
                            response = {"content": f"正在处理第 {element_count} 个元素的信息...",
                                        "is_task_complete": False}
                            yield response
            ...
            response = {"content": "处理请求时出现错误，请重试。",
                        "is_task_complete": True}
            yield response

    def get_agent_response(self, config: dict) -> ElementAPIResponse:
        """
        从智能体状态获取响应
        ...
        try:
            ...
            if structured_response and isinstance(structured_response, ElementResponse):
                ...
                elements = structured_response.elements
